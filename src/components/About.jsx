...
import image from "../assets/Hero-photo.webp";
...
const About = ({ isOpen, setIsOpen, isDark }) => {
  ...
  return createPortal(
    <>
      ...
      <div
        className={`fixed inset-2 z-50 flex max-h-[95vh] transform flex-col overflow-hidden rounded-3xl border-2 border-[#8E2DE2] transition-all duration-300 ease-in-out md:inset-10 md:max-h-[90vh] ${
          isDark ? "bg-[#191d26] text-white" : "bg-white text-gray-900"
        } ${isOpen ? "scale-100 opacity-100" : "scale-95 opacity-0"}`}
      >
        <button
          type="button"
          className="absolute top-4 right-4 z-20 cursor-pointer rounded-full border border-[#8E2DE2] bg-gray-800 p-2 text-[#8E2DE2] shadow-sm backdrop-blur-md transition-all duration-300 hover:rotate-90 hover:bg-white/20 md:top-6 md:right-6"
          aria-label="Close"
          onClick={() => setIsOpen(false)}
        >
          <svg
        ...
        <div className="h-full w-full overflow-y-auto p-5 md:p-10">
          <div className="relative flex min-h-full flex-wrap items-center justify-center py-4">
            <div
              className={`flex flex-col items-center md:w-[40%] md:border-r ${
                isDark ? "border-gray-600" : "border-gray-300"
              }`}
            >
              <div className="h-40 w-40 overflow-hidden rounded-full border-2 border-[#00BCD4] md:h-40 md:w-40">
                <img
                  src={image}
                  alt="Tanvir Hossain"
            ...
            <div className="mt-6 md:mt-0 md:w-[60%] md:pl-10">
              ...
              <div className="mt-4 flex flex-wrap gap-2 md:mt-6">
                {skills.map((framework, i) => (
                  <span
                    key={i}
                    className={`rounded-full border px-3 py-1 text-[12px] shadow-sm transition-all md:text-sm ${
                      isDark
